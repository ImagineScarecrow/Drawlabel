...
class Canvas(QtWidgets.QWidget):
    ...
    last_point = QtCore.Signal()#剩余最后一个点时触发该功能
    zoomRequest = QtCore.Signal(int, QtCore.QPoint)
    scrollRequest = QtCore.Signal(int, int)
    newShape = QtCore.Signal()
    rightClick = QtCore.Signal()#右键点击退出标注
    selectionChanged = QtCore.Signal(list)
    shapeMoved = QtCore.Signal()
    drawingPolygon = QtCore.Signal(bool)
    vertexSelected = QtCore.Signal(bool)
    deleteLabel = QtCore.Signal()#左键点击删除按钮
    CREATE, EDIT = 0, 1
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.num_backups = kwargs.pop("num_backups", 20)#原本是10
        ...
        self.scale = 1.0
        self.pixmap = QtGui.QPixmap()#加载图片
        self.visible = {}
    ...
    def isShapeRestorable(self):
        ...
        #我们在每次编辑之后(而不是在编辑之前)保存状态，所以为了一个编辑是可撤消的，我们期望当前和先前的状态在撤消堆栈中。
        if len(self.shapesBackups) < 2:#2
            return False
        return True
    ...
    def restoreShape(self):
        ...
        #应用程序最终将调用Canvas。loadShapes，它会把这个推回到堆栈上。
        ...
        if self.drawing():
            ...
            if not self.current :
                if(self.createMode == "rectangle"):
                    self.prevPoint = pos#画矩形框的时候添加辅助线
                    self.update()
                return
            ...
            if self.drawing():
                if self.current:
                    ...
                    if self.createMode == "point":
                        ...
                        self.setHiding()
                        self.drawingPolygon.emit(True)#画图的时候发送信号，禁止其他模式触发
                        self.update()#触发绘图事件
            elif self.editing():
                #self.deleteLabel.emit()
                if self.selectedEdge():
                    self.addPointToEdge()
                else:#点击之后变成手
                    self.overrideCursor(CURSOR_GRAB)
                    QtWidgets.QApplication.setOverrideCursor(QtGui.QCursor(Qt.OpenHandCursor))
                    #print("i openCrusor")
...
#and self.editing()
    ...
    def paintEvent(self, event):
        ...
        if self.drawing() and not self.prevPoint.isNull() and not self.outOfPixmap(self.prev_point)and self.createMode=="rectangle":
